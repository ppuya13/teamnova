...
//                    System.out.println(상점.상점구매타겟(상점구매)+"을(를) 구매했습니다.");//변수 '상점구매' 값에 따라 품목명을 다르게 넣기
                ...
                if (화면 == 3 && 세부1 == 1 && 세부2 == 1 && 스택가능==1 && b>=1 && b<=상점.리스트.size()) { // 스택가능한 아이템을 선택했을 때, 1이상이고 상점리스트개수 이하의 값을 입력했을 경우
                    세부2 = 0;
                    구매개수 = b;
                    상점구매확인1 = true;
                    b = -1;
                ...
                if (화면 == 3 && 세부1 == 2 && 세부2 == 0 && b >= 1 && b <= 캐릭터.소지품.size()+캐릭터.회복물약가방.size()) { // 320에서 입력값이 1이상이며 소지품 개수 이하일 때
                    세부2 = 1;
                    상점판매 = b; //뭘 눌러서 구매확인창이 떴는지 확인
                    b = -1;
                    if(상점판매<=캐릭터.회복물약가방.size()){ //상점판매 값이 1이상이고 회복물약가방의 사이즈 이하면(회복물약가방의 내용물이면)
                        System.out.println("회복물약가방 이프문 진입");
                        물약여부=1;
                        스택가능=1; //선택한 아이템의 스택가능여부가 true일 경우 스택가능을 1로
                    }
                    else {//상점판매 값이 3이상이고 소지품 사이즈 이하면
                        System.out.println("상점판매 : "+상점판매 + ", 상점구매 : "+상점구매 + ", 캐릭터.회복물약가방.size() : " + 캐릭터.회복물약가방.size() + ", 캐릭터.소지품.size() : "+캐릭터.소지품.size());
