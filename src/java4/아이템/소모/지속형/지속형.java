...
//        System.out.println("(지속형.사용효과) if문 진입");
        if(플레이어.사용중.size() != 0) { //사용중인 템이 있다면
            for (int i = 0; i < 플레이어.사용중.size() ; i++){ //사용중 크기만큼 반복
                if(소모템.아이템이름.equals(플레이어.사용중.get(i).아이템이름)){ //사용중인 아이템 중에 선택된 아이템과 동일한 템이 있으면
//                    System.out.println("(지속형.사용효과) 동일한템있음");
                    플레이어.사용중.set(i,소모템); //그 아이템을 선택한 아이템으로 대체
                    사용완료=true;
                    this.스택수--;
                }
            }
...
//            System.out.println("(아이템.소모템사용) 사용중인템없음");
    ...
    public abstract void 효과적용(플레이어 플레이어) throws InterruptedException;

    public abstract boolean 효과삭제(플레이어 플레이어) throws InterruptedException;
}
